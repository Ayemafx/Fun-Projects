...
def create_window():
    ...
    button_size = (2, 1)
    layout = [
        [sg.Push(), sg.Button('x', size=button_size, key='-CLOSE-', enable_events=True)],
        [sg.VPush()],
...
while True:
    ...
    if event == '-LAP-':
        window.extend_layout(window['-COLUMN-'], [[sg.Text(lap_amount), sg.VSeparator(), sg.Text(elapsed_time)]])
        lap_amount += 1
window.close()
